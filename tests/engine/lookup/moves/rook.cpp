...
TEST_CASE("chester::engine::lookup::moves<piece::rook>()", "[.][engine][lookup][rook]") {
    square   square;
    bitset blockers;
    bitset expected;

    std::tie(square, blockers, expected) =
        GENERATE(table<class square, bitset, bitset>({
            std::make_tuple(
                square::a8,
                bitset::empty(),
                square::a1 |
    ...
    const bitset found = chester::engine::lookup::moves<piece_type::rook>(square, blockers);
